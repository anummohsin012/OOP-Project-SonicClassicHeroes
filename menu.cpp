...
public:
	Menu(int height, int width) : optionsopen(false), index(0), volumelevel(0) {
		...
		title.setPosition(width -260 , height/6) ;
		vol.setFont(font);
		...
		for (int i = 0;i < 5;i++) {
			...
			menuoptions[i].setPosition(width -100, (height) /3 -50 +i*80);
	...
	void draw(sf::RenderWindow& window, int height, int width) {
		...
		void nameinput(RenderWindow& window, int height, int width) {
			...
			bool goBackToMenu = false; // Flag to go back to menu
			...
			while (window.isOpen()) {
				...
				while (window.pollEvent(event)) {
					...
					if (event.type == Event::TextEntered) {
						if (event.text.unicode == 8 && playerName.length() > 0) { // Backspace
							playerName.pop_back();
						}
						else if (event.text.unicode < 128) { // Valid ASCII character
				...
				nameDisplay.setString(playerName); // Update the display
				window.clear();
				window.draw(menusp);
				window.draw(name);
				window.draw(nameDisplay);
				...
				if (event.type == Event::KeyPressed && event.key.code == Keyboard::Escape) {
					goBackToMenu = true; // Set flag to return to the menu
				}

				if (event.type == Event::KeyPressed && event.key.code == Keyboard::Enter) {
					// Once Enter is pressed, start the game (here you can add logic for saving the player's name)
